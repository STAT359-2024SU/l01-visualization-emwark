...
This plot is generally vague in what it actually depicts. Any further relationships between individual city mpg levels, hwy mpg, and car type is obscured by the apparent positive linear relationship. We could try sorting by some detail about the cars-- year, manufacturer, model, etc, to explore whether there are further relationships.
:::
...
```
```{r}
# one option looking at year and displ
ggplot(mpg, aes(x = hwy, y = cty, color = year, size = displ)) +
  geom_point()
```
```{r}
# another option just looking at size = displ
ggplot(mpg, aes(x = hwy, y = cty, size = displ)) +
  geom_point()
```

```{r}
# shape does not work for continuous data
ggplot(mpg, aes(x = hwy, y = cty, color = year, shape = drv)) +
  geom_point()
```
```{r}
#  option with shape = displ, a numerical variable, which gives an error because it is a continuous variable.
# ggplot(mpg, aes(x = hwy, y = cty, shape = displ)) +
#  geom_point()
```
...
```{r}
ggplot(tinder_data, aes(x = messages_sent, group = user_gender)) +
  geom_histogram()
```
